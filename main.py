...
LOG_CHANNEL_ID = 1367958714379927693 SERVER_ID = 1324363465745240118 ROL_ID = 1327354131181994004 translator = Translator() pending_users = {} recent_clicks = {}  # === Nou: pentru prevenirea click-urilor multiple ===

=== View persistentă ===

class ButonAcces(discord.ui.View): def init(self): super().init(timeout=None)
...
async def buton_acces(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    if user_id in recent_clicks and now - recent_clicks[user_id] < 30:
        await interaction.response.send_message("Te rog așteaptă puțin înainte de a apăsa din nou.", ephemeral=True)
        return
...
class HelpButtons(discord.ui.View): @discord.ui.button(label="Metode de plată", style=discord.ButtonStyle.primary) async def metode_plata(self, interaction: discord.Interaction, button: discord.ui.Button): await interaction.response.send_message( "Poți plăti prin:\n- Revolut: https://revolut.me/liliancj2v\n- PayPal: https://www.paypal.me/RomaniaQuiz\n- Transfer bancar: IBAN: RO56BTRLRONCRT0CQ2528301", ephemeral=True)
...
@discord.ui.button(label="Întrâmpin dificultăți", style=discord.ButtonStyle.danger)
async def dificultati(self, interaction: discord.Interaction, button: discord.ui.Button):
    log_channel = bot.get_channel(LOG_CHANNEL_ID)
    if log_channel:
        await log_channel.send(f"{interaction.user.mention} a apăsat pe 'Întrâmpin dificultăți'.")
    await interaction.response.send_message("Owner-ul serverului va fi notificat să te contacteze.", ephemeral=True)

@bot.command() async def buton_acces(ctx): await ctx.send("Dacă ești interesat, apasă pe butonul de mai jos sau mesaj în privat.", view=ButonAcces())

@bot.event async def on_ready(): print(f"Botul este online ca {bot.user}") bot.add_view(ButonAcces())
...
if isinstance(message.channel, discord.DMChannel):
    ...
    if any(cuv in mesaj for cuv in ["am plătit", "am platit", "gata", "am trimis", "am dat"]):
        ...
        await message.channel.send(
            ...
            "Dacă întrâmpini probleme, contactează Owner-ul serverului.\n\n"
            "Până atunci te pot ajuta cu:", view=HelpButtons())
        return
...
async def start_follow_up_timer(message): user_id = message.author.id if user_id in pending_users: pending_users[user_id]["task"].cancel()
